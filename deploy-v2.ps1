...
Write-Host "🚀 Desplegando versión 2 de Hotel & Casino API..." -ForegroundColor Yellow
...
Write-Log "🔍 PASO 1: Verificando que backend_v1 esté activo..."
try {
    $response = Invoke-WebRequest -Uri "http://localhost:3000" -Method GET -TimeoutSec 10
    Write-Log "✅ backend_v1 funcionando correctamente"
} catch {
    Write-Error "backend_v1 no está funcionando en localhost:3000. Ejecuta .\deploy.ps1 primero"
}

# PASO 2: Desplegar backend_v2
Write-Log "🏗️ PASO 2: Desplegando backend_v2 (nueva versión)..."
docker-compose --profile v2 up -d backend_v2

# PASO 3: Esperar que v2 esté lista
Write-Log "⏳ PASO 3: Esperando que backend_v2 esté lista en localhost:3001..."
...
do {
    ...
    $counter += 3
    if ($counter -ge 45) {
        Write-Log "⚠️ backend_v2 no responde, manteniendo v1 activa"
...
Write-Host ""
Write-Log "✅ backend_v2 está respondiendo!"
...
try {
    ...
    if ($v1_response.Content -match '"version":"1.0.0"' -and $v2_response.Content -match '"version":"2.0.0"') {
        ...
        Write-Log "⚠️ Error en las versiones. Limpiando v2..."
        docker-compose stop backend_v2
        Write-Error "Las versiones no responden correctamente"
    }
} catch {
    Write-Log "⚠️ Error verificando versiones. Limpiando v2..."
    docker-compose stop backend_v2
    Write-Error "Error en la verificación de versiones"
}
...
Write-Host "   🗄️ Base de datos compartida → localhost:5432" -ForegroundColor White
Write-Host ""
...
Write-Host ""
Write-Host "⚠️ Si hay problemas con v2, ejecuta: .\rollback.ps1" -ForegroundColor Yellow
