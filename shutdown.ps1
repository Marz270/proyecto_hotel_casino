...
try {
    docker network rm proyecto_hotel_casino_hotel-network 2>$null
...
try {
    docker volume rm proyecto_hotel_casino_db_data 2>$null
...
docker ps -a --filter "name=hotel_"

Write-Host "`nImágenes del proyecto:" -ForegroundColor Yellow
docker images | Select-String "proyecto_hotel_casino"